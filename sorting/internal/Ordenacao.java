...
import sorting.Item; // @{\it vide Programa~\ref{prog:interfaceitem}}@
...
public class Ordenacao {
  ...
  //selecao para o exercicio 26. So mudou o tipo do vetor que era de Items e agora eh de ItemCNHs. Ver classes ItemCNH e CNH.
  public static void selecao (ItemCNH v[], int n) {
    for (int i = 1; i <= n - 1; i++) {
      int min = i;
      for (int j = i + 1; j <= n; j++)
        if (v[j].compara (v[min]) < 0) min = j;
      ItemCNH x = v[min]; v[min] = v[i]; v[i] = x;
    }
  }
  
  //Exercicio 24 para o selecao
  public static void selecaoInvertido (Item v[], int n) {
    for (int i = 1; i <= n - 1; i++) {
      int max = i;
      for (int j = i + 1; j <= n; j++)
        if (v[j].compara (v[max]) >= 0) max = j;
      Item x = v[max]; v[max] = v[i]; v[i] = x;
    }
  }
  
  //Exercicio 25 para o selecaoParcial
  public static void selecaoParcial (Item v[], int n, int limite) {
  ...
  //Exercicio 24 para o insercao
  public static void insercaoInvertido (Item v[], int n) {
    int j;
    for (int i = 2; i <= n; i++) {
      Item x = v[i]; 
	  j = i - 1;
  ...
  //Exercicio 25 para o insercaoParcial
  public static void insercaoParcial (Item v[], int n, int limite) {
    int j;
